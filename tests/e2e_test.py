...
@pytest.mark.asyncio
async def test_api_server_meta_http(host: str, enable_grpc: bool) -> None:
    if enable_grpc:
        pytest.skip("Skip gRPC tests for HTTP e2e tests.")
...
async def test_inference_http(
    host: str, text: str, categories: list[str], enable_grpc: bool
):
    if enable_grpc:
        pytest.skip("Skip gRPC tests for HTTP e2e tests.")
...
async def test_inference_grpc(
    ...
    if not enable_grpc:
        pytest.skip("Skip HTTP tests for gRPC e2e tests.")
