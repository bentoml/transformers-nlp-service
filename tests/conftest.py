...
@pytest.fixture(name="text", params=[TEXT], scope="session")
def fixture_text(request: FixtureRequest):
    return request.param


@pytest.fixture(name="categories", params=[CATEGORIES], scope="session")
def fixture_categories(request: FixtureRequest):
    return request.param
...
def skip_server_type(request: FixtureRequest, enable_grpc: bool):
    if request.node.get_closest_marker("skip_server_type") is not None:
        skip_args = request.node.get_closest_marker("skip_server_type").args[0]
        if (skip_args == "grpc" and not enable_grpc) or (
            skip_args == "http" and enable_grpc
        ):
            pytest.skip(f"Skip {skip_args} Server for the given tests.")
